...
export async function GET(req, context) {
  ...
  try {
    const p = await prisma.Intern.findUnique({ where: { id: numericId } });
    if (!p) return NextResponse.json({ error: "Not found" }, { status: 404 });
    return NextResponse.json(p);
  } catch (err) {
    console.error("GET /api/Intern/[id] error:", err);
...
export async function POST(req, context) {
  ...
  try {
    ...
    console.error("POST /api/Interns/[id] error:", err);
...
export async function DELETE(req, context) {
  ...
  try {
    await prisma.Intern.delete({ where: { id: numericId } });
    return new Response(null, { status: 204 });
  } catch (err) {
    console.error("DELETE /api/Interns/[id] error:", err);
